=== Horizontalee ===
Contributors: leemon
Version: 3.0
Requires at least: 7.1
Tested up to: 7.1
Requires PHP: 7.4
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Tags: block-styles, blog, custom-colors, custom-logo, custom-menu, e-commerce, left-sidebar, photography, portfolio, right-sidebar, theme-options, translation-ready, threaded-comments, two-columns 

== Description ==
Horizontalee is a lightweight, responsive and highly flexible WordPress theme based on the underscores framework. It features multiple template parts, navigation menus at the top and more. It has been written from ground up with SEO in mind, is compatible with the block editor and follows the latest web standards. With its minimalistic approach and lots of customization options, it enables you to build almost any type of website such as a blog, a portfolio, a corporate site or an online shop.

== Installation ==

1. Upload the extracted theme folder and contained files to your `/wp-content/themes/` directory
2. Click on the 'Activate' button to use your new theme right away.
3. Navigate to Appearance > Customize in your admin panel and customize to taste.

== Copyright ==

Horizontalee is distributed under the terms of the GNU GPL

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

Horizontalee Theme bundles the following third-party resources:

Material icons, Copyright Google
License: Apache License, Version 2.0
Source: https://material.io/

Font Awesome icons, Copyright Dave Gandy
License: SIL Open Font License, Version 1.1
Source: https://fontawesome.com/

Alpha Color Picker Customizer Control, Copyright 2015 Braad Martin
License: GNU GPL, Version 2 (or later)
Source: https://github.com/BraadMartin/components/tree/master/customizer/alpha-color-picker

normalize.css, Copyright 2012-2018 Nicolas Gallagher and Jonathan Neal
License: MIT License
Source: https://necolas.github.io/normalize.css/

Underscores, Copyright 2012-2018 Automattic, Inc.
License: GNU GPL, Version 2 (or later)
Source: https://underscores.me/

== Changelog ==

= 3.0 =
* Released: February 17, 2022

= 2.0 =
* Released: February 17, 2021

= 1.0 =
* Released: December 10, 2016

Initial release
